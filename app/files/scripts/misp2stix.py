...
def loadEvent(args, pathname):
    try:
        ...
        print((json.dumps({'file':filename, 'ex': str(ex), 'success' : 0, 'message' : 'The temporary MISP export file could not be read'})))
        sys.exit(1)
...
def generateMainPackage(events):
    ...
    stix_header.title="Export STIX from " + namespace[1] + " MISP"
...
def generateEventPackage(event):
    ...
    stix_header = STIXHeader()
    stix_header.title= "HEY HEY" + event["Event"]["info"] + " (MISP Event #" + event["Event"]["id"] + ")"
    #stix_header.package_intents="Threat Report"
    stix_package.stix_header = stix_header
